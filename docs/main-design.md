...
将流程中用于计算处理的组件分为5类：[Loader](./loader.md)、[Processor](./processor.md)、Database、util、gestata。

- Loader 用于数据加载，是流程的数据输入。典型输入包括文件、数据库、消息队列等。
- Processor 用于数据处理，包括流程的数据输出。典型处理和输出包括格式转换、过滤、选择以及文件输出、数据库输出等。
- Database 数据库组件。用于对接各类数据库，可支持查询（search）、遍历（scroll）、获取详情（get）、写入（upsert）、删除（delete）等操作，具体支持操作类型由特定数据库决定。数据库是ETL中流程最基础最重要的组件。
- util 工具方法，定义一组基础工具方法供调用，例如日期计算、字符串处理等。
- gestata 根据业务需要定义的计算处理组件

组件python源码放置在对应的模块下面，例如，util位于smartetl/util模块中。
